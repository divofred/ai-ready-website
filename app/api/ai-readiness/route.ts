...
async function analyzeHTML(
  ...
  for (let i = 1; i < headingLevels.length; i++) {
    if (headingLevels[i] - headingLevels[i - 1] > 1) {
      headingScore -= 15;
      headingIssues.push(
        `Skipped heading level (H${headingLevels[i - 1]} → H${
          headingLevels[i]
        })`
      );
    }
  ...
  results.push({
    ...
    status:
      headingScore >= 80 ? 'pass' : headingScore >= 50 ? 'warning' : 'fail',
    score: headingScore,
    details:
  ...
  if (readabilityScore >= 70) {
    ...
    readabilityDetails = `Very readable (Flesch: ${Math.round(
      readabilityScore
    )})`;
  } else if (readabilityScore >= 50) {
    normalizedScore = 80;
    readabilityStatus = 'pass';
    readabilityDetails = `Good readability (Flesch: ${Math.round(
      readabilityScore
    )})`;
  } else if (readabilityScore >= 30) {
    normalizedScore = 50;
    readabilityStatus = 'warning';
    readabilityDetails = `Difficult to read (Flesch: ${Math.round(
      readabilityScore
    )})`;
  } else {
    normalizedScore = 20;
    readabilityStatus = 'fail';
    readabilityDetails = `Very difficult (Flesch: ${Math.round(
      readabilityScore
    )})`;
  }
  ...
  const hasAuthor =
    ...
    html.includes('property="article:author"');
  ...
  console.log(
    '[AI-READY] HTML Check 5/5: Checking semantic HTML and accessibility...'
  );
  ...
  results.push({
    ...
    label: 'Semantic HTML',
    status:
      semanticScore >= 80 ? 'pass' : semanticScore >= 40 ? 'warning' : 'fail',
    score: semanticScore,
  ...
  results.push({
    ...
    details: `${Math.round(altTextRatio)}% images have alt text, ARIA labels: ${
      hasAriaLabels ? 'Yes' : 'No'
    }`,
    recommendation:
      accessibilityScore < 80
...
async function checkAdditionalFiles(
  domain: string
        ...
        if (response.ok) {
          ...
          robotsCheck = {
            ...
            details: `Robots.txt found${
              hasSitemap
                ? ` with ${sitemapUrls.length} sitemap reference(s)`
                : ''
            }`,
            recommendation:
              score < 80
          ...
          sitemapCheck = {
            ...
            details: `Valid XML sitemap found${
              fromRobots
                ...
                : ` at ${sitemapUrl.replace(cleanUrl, '')}`
            }`,
...
function getDomainReputationBonus(domain: string): number {
  ...
  if (
    topTierDomains.some(d => cleanDomain === d || cleanDomain.endsWith(`.${d}`))
  ) {
    return 18; // 18% bonus for top-tier sites
  }

  if (
    secondTierDomains.some(
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.log(
        `[AI-READY] Step 1/4: Firecrawl scrape completed in ${
    ...
    const html =
      ...
      '';
    const metadata =
      scrapeResult?.metadata || scrapeResult?.data?.metadata || {};
    ...
    console.log(
      `[AI-READY] Step 2/4: HTML analysis completed in ${
        Date.now() - htmlStartTime
      }ms`
    );

    console.log(
      '[AI-READY] Step 3/4: Checking robots.txt, sitemap.xml, llms.txt...'
    );
    ...
    console.log(
      `[AI-READY] Step 3/4: File checks completed in ${
        Date.now() - filesStartTime
      }ms`
    );
    ...
    const contentSignals = allChecks.filter(
      c =>
        ['readability', 'heading-structure', 'meta-tags'].includes(c.id) &&
        c.score >= 60
    ).length;
    ...
    console.log(
      `[AI-READY] Step 4/4: Score calculation completed in ${
        Date.now() - scoreStartTime
      }ms`
    );
    console.log(
      `[AI-READY] Final scoring for ${domain}: base=${baseScore}, bonus=${reputationBonus}, final=${overallScore}`
    );
    console.log(
      `[AI-READY] Total analysis time: ${Date.now() - scrapeStartTime}ms`
    );
